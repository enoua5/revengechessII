...
PieceIdentifier Board::getPlayField(unsigned char i) const
{
  if (i >= 64)
    return EMPTY;
  return playField[i];
}
Piece Board::getBoardPiece(unsigned char i) const
{
  if (i >= 32)
    return Piece();
  return pieces[i];
...
MoveInfo Board::makeMove(const Move move, bool trusted) const
{
  ...
  if (pawnMove)
  {
    ...
    else if ((move.from.rank == 1 && move.to.rank == 3) || (move.from.rank == 6 && move.to.rank == 4))
      next.pawnEnPassantFile = (File)move.to.file;
    else
      next.pawnEnPassantFile = NONE;
  }
  else
    next.pawnEnPassantFile = NONE;
